...
TRANSLATIONS = {
    "English": {
        ...
        "greeting": "Good morning!", "next_dose": "Next dose",
    ...
    "Hindi": {
        ...
        "greeting": "सुप्रभात!", "next_dose": "अगली खुराक",
    ...
    "Tamil": {
        ...
        "greeting": "காலை வணக்கம்!", "next_dose": "அடுத்த மருந்து",
    ...
    "Malayalam": {
        ...
        "greeting": "സുപ്രഭാതം!", "next_dose": "അടുത്ത ഡോസ്",
    ...
    "Bengali": {
        ...
        "greeting": "শুভ সকাল!", "next_dose": "পরবর্তী ডোজ",
    ...
    "Urdu": {
        ...
        "greeting": "صبح بخیر!", "next_dose": "اگلی خوراک",
    ...
    "Nepali": {
        ...
        "greeting": "शुभ प्रभात!", "next_dose": "अर्को मात्रा",
    ...
    "French": {
        ...
        "greeting": "Bonjour !", "next_dose": "Prochaine dose",
...
if "taken_today" not in st.session_state:
    st.session_state.taken_today = False
...
with tab_home:
    ...
    all_medicine_names = sorted(df["name"].dropna().unique())

    if "home_medicine" not in st.session_state:
        st.session_state.home_medicine = all_medicine_names[0]

    home_medicine_name = st.selectbox(
        t("select_medicine"), all_medicine_names, key="home_medicine"
    )
    ...
    checked = st.checkbox(t("took_this"), value=st.session_state.taken_today)

    if checked and not st.session_state.taken_today:
        # Just got checked this run — confirm it, then move on to the next medicine
        st.toast(t("marked_taken"))
        current_index = all_medicine_names.index(home_medicine_name)
        next_index = (current_index + 1) % len(all_medicine_names)
        st.session_state.home_medicine = all_medicine_names[next_index]
        st.session_state.taken_today = False
        st.rerun()
    else:
        st.session_state.taken_today = checked
    ...
    other_meds = df[df["name"] != home_medicine_name]
    for _, row in other_meds.iloc[0:2].iterrows():
        st.write(f"🕑 — **{row['name']}**")
