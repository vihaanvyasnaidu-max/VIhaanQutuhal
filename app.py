...
def speak(text, filename="voice.mp3"):
    ...
    try:
        ...
        st.error("Could not play audio right now. Check your internet connection.")
...
def show_medicine_card(row, key_prefix):
    ...
    with col2:
        if st.button(t("listen"), key=f"listen_{key_prefix}_{row['id']}", use_container_width=True):
            speak(f"{row['name']} contains {row['short_composition1']}. The price is {row['price']:.0f} rupees.")
    ...
    for alt_col, price_col in ALT_PAIRS:
        ...
        with colB:
            if st.button(t("listen"), key=f"listen_{key_prefix}_{alt_col}_{row['id']}", use_container_width=True):
                speak(f"{alt_name} is an alternative, priced at {alt_price} rupees.")
        st.markdown('</div>', unsafe_allow_html=True)
...
with tab_home:
    ...
    st.write(f"**{next_dose['name']}** — 1 dose")
    st.markdown('</div>', unsafe_allow_html=True)

    col1, col2 = st.columns(2)
    with col1:
        if st.button(t("took_this"), use_container_width=True):
            st.session_state.taken_today = True
    with col2:
        if st.button(t("read_aloud"), use_container_width=True):
            speak(f"It is time to take {next_dose['name']}, one dose.")
...
with tab_find:
    ...
    medicine_names = sorted(df["name"].dropna().unique())
