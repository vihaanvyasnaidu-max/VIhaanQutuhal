...
if not st.session_state.logged_in:
    ...
    if submitted:
        ...
        phone_valid = len(phone_digits) >= 7  # a loose check, not a strict format
...
with tab_home:
    ...
    if st.session_state.show_add_form:
        with st.container(border=True):
            ...
            timing_choice = st.radio(
                t("timing_question"), [t("before_food"), t("after_food")],
                key="home_manual_timing", horizontal=True,
            )
            minutes_choice = st.number_input(
                t("minutes_label"), min_value=0, max_value=180, value=30, step=5,
                key="home_manual_minutes",
            )
            time_choice = st.time_input(
                t("time_of_day_label"), value=datetime.time(8, 0), key="home_manual_time"
            )
            if st.button(t("add_button"), key="home_manual_add_btn", use_container_width=True):
                ...
                if new_name.strip() and new_name.strip() not in existing_names:
                    st.session_state.my_medicines.append({
                        "name": new_name.strip(),
                        "timing": timing_choice,
                        "minutes": minutes_choice,
                        "time": time_choice,
                    })
                    st.session_state.show_add_form = False
            ...
            if photo is not None:
                ...
                if matches:
                    ...
                    for name in matches:
                        ...
                        elif st.session_state.get("scan_detail_target") == name:
                            # This match's detail form is open — collect timing, then confirm.
                            timing_choice = st.radio(
                                t("timing_question"), [t("before_food"), t("after_food")],
                                key=f"scan_timing_{name}", horizontal=True,
                            )
                            minutes_choice = st.number_input(
                                t("minutes_label"), min_value=0, max_value=180, value=30, step=5,
                                key=f"scan_minutes_{name}",
                            )
                            time_choice = st.time_input(
                                t("time_of_day_label"), value=datetime.time(8, 0), key=f"scan_time_{name}"
                            )
                            if st.button(t("confirm_button"), key=f"scan_confirm_{name}", use_container_width=True):
                                st.session_state.my_medicines.append({
                                    "name": name, "timing": timing_choice,
                                    "minutes": minutes_choice, "time": time_choice,
                                })
                                st.session_state.scan_detail_target = None
                                st.toast(t("custom_added"))
                                st.rerun()
                        else:
                            if st.button(t("add_button"), key=f"scan_add_{name}", use_container_width=True):
                                st.session_state.scan_detail_target = name
                                st.rerun()
                        st.divider()
                else:
    ...
    if not my_medicines:
        ...
        st.caption(
            ...
            f"{current_details['time'].strftime('%I:%M %p')}"
        )
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if other_meds:
            ...
            for name in other_meds:
                ...
                st.write(
                    ...
                    f"{other_details['time'].strftime('%I:%M %p')})"
                )
