...
<style>
    ...
    input[type="checkbox"] { accent-color: #0F6E56 !important; transform: scale(1.4); margin-right: 4px; }
    div[data-testid="stCheckbox"] label span[data-testid="stMarkdownContainer"] p { font-size: 20px !important; }
    div[data-testid="stCheckbox"] svg { fill: #0F6E56 !important; }
    .med-card {
...
with tab_home:
    ...
    if "home_medicine" not in st.session_state:
        st.session_state.home_medicine = df["name"].dropna().unique()[0]

    home_medicine_name = st.selectbox(
        t("select_medicine"), sorted(df["name"].dropna().unique()), key="home_medicine"
    )
    ...
    st.session_state.taken_today = st.checkbox(t("took_this"), value=st.session_state.taken_today)

    if st.button(t("read_aloud"), use_container_width=True):
        speak(t("speech_reminder").format(name=next_dose['name']))

    if st.session_state.taken_today:
