...
with tab_home:
    ...
    if st.session_state.show_add_form:
        with st.container(border=True):
            ...
            if st.button(t("add_button"), key="home_manual_add_btn", use_container_width=True):
                if new_name.strip() and new_name.strip() not in st.session_state.my_medicines:
                    st.session_state.my_medicines.append(new_name.strip())
                    st.session_state.show_add_form = False
        ...
        with st.container(border=True):
            ...
            if photo is not None:
                ...
                if matches:
                    ...
                    for name in matches:
                        already_added = name in st.session_state.my_medicines
                        col_name, col_add_match = st.columns([3, 1])
                        with col_name:
                            st.write(f"**{name}**")
                        with col_add_match:
                            if st.button(
                                t("add_button"), key=f"scan_add_{name}",
                                disabled=already_added, use_container_width=True,
                            ):
                                st.session_state.my_medicines.append(name)
                                st.toast(t("custom_added"))
                                st.rerun()
                else:
    ...
    if not my_medicines:
        ...
        if "home_medicine" not in st.session_state or st.session_state.home_medicine not in my_medicines:
            st.session_state.home_medicine = my_medicines[0]

        home_medicine_name = st.selectbox(
            t("select_medicine"), my_medicines, key="home_medicine"
        )
        ...
        if checked:
            if len(my_medicines) > 1:
                ...
                # loop endlessly.
                current_index = my_medicines.index(home_medicine_name)
                next_index = (current_index + 1) % len(my_medicines)
                st.toast(t("marked_taken"))
                st.session_state.pending_home_medicine = my_medicines[next_index]
                st.rerun()
            else:
        ...
        other_meds = [m for m in my_medicines if m != home_medicine_name]
        if other_meds:
            st.divider()
