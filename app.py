...
with tab_home:
    ...
    # ---- Meal-time settings, used to trigger automatic voice reminders ----
    ...
    with st.expander(t("meal_times_label")):
        st.caption(t("auto_reminder_note"))
        col_b, col_l, col_d = st.columns(3)
        with col_b:
            st.session_state.meal_times["Breakfast"] = st.time_input(
                t("meal_breakfast"), value=st.session_state.meal_times["Breakfast"], key="mealtime_breakfast"
            )
        with col_l:
            st.session_state.meal_times["Lunch"] = st.time_input(
                t("meal_lunch"), value=st.session_state.meal_times["Lunch"], key="mealtime_lunch"
            )
        with col_d:
            st.session_state.meal_times["Dinner"] = st.time_input(
                t("meal_dinner"), value=st.session_state.meal_times["Dinner"], key="mealtime_dinner"
            )
